...
const { mapErrors } = require('./error-mapping');
const { formatError } = require('./error-formatter');
const { getFullService } = require('./utils/get-service-name');
  ...
  actions: {
    rest: {
      ...
      tracing: {
        ...
        spanName: (context) => `${context.data.request.method.toUpperCase()} ${context.data.request.url}`
      },
      handler (context) {
  ...
  methods: {
    ...
    sendResponse (context, request, response, action, data) {
      ...
      if (Buffer.isBuffer(data)) {
        ...
        result = data;
      } else if (isObject(data) && data.type === 'Buffer') { // Handle serialized Buffer
        // Buffer from object
    ...
    sendError (request, response, error) {
      ...
      if (this.settings.errorHeader && isFunction(this.settings.errorHeader)) {
        this.settings.errorHeader.call(this, error, response);
      } else {
        response.setHeader('Content-type', 'application/json; charset=UTF-8');
      }

      const code = mapErrors(error, this);

      this.logResponse(request, response, error ? error.context : null);
      response.writeHead(code);
      response.end(formatError(code, error, this));
    },
    ...
    regenerateRoutes () {
      this.registeredHandlers.map(handler => handler.schema.generateRoutes && this.generateRoutesForServices(handler));
    },
    generateRoutesForServices (handler) {
      ...
      services.map(service => {
        ...
        const serviceName = services.fullName || getFullService(service); // todo generate name from version
        const basePath = service.settings && service.settings.http && isString(service.settings.http.basePath) ? service.settings.http.basePath : addSlashes(serviceName);
        ...
        Object.values(service.actions).map(action => {
          ...
          if (isString(action.http)) {
            const result = parseRouteSchemaString(action.http);
            routeSchema = {
              method: result.method,
              path: basePath + result.path
            };
          } else if (isPlainObject(action.http)) {
            routeSchema = {
              method: action.http.method,
              path: basePath + action.http.path
            };
          }
