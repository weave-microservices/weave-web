...
function logRouteRegistration (schema, routeHandler, message) {
  const { log, settings } = routeHandler.service;
  if (settings.logRouteRegistration && settings.logRouteRegistrationLevel in log) {
    log[settings.logRouteRegistrationLevel](message);
  }
}
...
exports.createRoute = (routeHandler, schema, action) => {
  ...
  logRouteRegistration(schema, routeHandler, `Register route '${route.fullPath}'`);
