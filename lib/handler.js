  ...
  constructor (service, schema) {
    ...
    if (service.settings.rateLimit && service.settings.rateLimit.enabled || schema.rateLimit && schema.rateLimit.enabled) {
      ...
      if (opts.RateLimitStore) {
        this.rateLimit.store = new opts.RateLimitStore(opts.windowSizeTime, opts)
      } else {
    ...
    this.mappingPolicy = schema.mappingPolicy || service.settings.mappingPolicy || MAPPING_POLICY_RESTRICTED
    ...
    return new Promise((resolve, reject) => {
      ...
      return this.wrapMiddlewaresPromisified(request, response, this.middlewares)
        .then(() => {
          ...
          if (this.cors) {
            writeCorsHeaders(this, request, response, true)
