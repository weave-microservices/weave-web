...
module.exports = () => ({
  ...
  settings: {
    ...
    useHttp2: false,
    routing: {
      historyMode: false // todo: evaluate a better implementation
    },
    routes: [
      {
        path: '/',
      ...
      handler (context) {
        ...
        for (let i = 0; i < this.routes.length; i++) {
          ...
          if (url.startsWith(route.path)) {
            return route.handle(context, request, response)
          }
        }
  ...
  methods: {
    ...
    addRoute (routeSchema, addToBottom = true) {
      const newRoute = new Route(this, routeSchema)
      ...
      if (route.rateLimit) {
        ...
        if (key) {
          ...
          if (remainingRequests < 0) {
            return Promise.reject(new RateLimitExceededError()) // this.sendError(request, response, new RateLimitExeededError())
          }
        }
      }
      ...
      return Promise.resolve()
        ...
        .then(() => {
          if (route.beforeRequest) {
            return route.beforeRequest.call(
              this,
              context,
              route,
              request,
      ...
      return Promise.resolve()
        ...
        .then(data => {
          if (route.afterRequest) {
            return route.afterRequest(context, route, request, response, data)
          }
          return data
        })
        .then(data => {
    ...
    setHeaders (request, response) {
      if (!response.getHeader('Connection')) {
        response.setHeader(
          'Connection',
          request.isKeepAlive() ? 'keep-alive' : 'close'
        )
      }
    },
    ...
    sendError (request, response, error) {
      if (isFunction(request.$route.requestFailed)) {
        return request.$route.requestFailed.call(this, request, response, error)
      }
    ...
    logResponse (request, response) {
      ...
      this.log.debug(
        `<= ${response.statusCode} ${request.method} ${request.url} ${durationString}`
      )
    },
    closeAllConnections () {
      Object.entries(this.connections).forEach(([key, connection]) =>
        connection.destroy()
      )
    },
    tryCreateHttp2 () {
